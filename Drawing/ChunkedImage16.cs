...
{
	/// <summary>
	/// Describes a chunked image of 1, 3 (RGB) or 4 (RGBX) unsigned 16-bit integers per pixel.
	/// </summary>
	[CLSCompliant(false)]
	public class ChunkedImage16 : TiledImage16, ChunkedImage
	{
		#region Properties
		...
		public ChunkedImage16(int width, int height, bool RGB, bool extraChannel, bool init, int chunkHeight = 256, PlanarConfiguration planarConfig = PlanarConfiguration.Continuously):
		...
		/// <param name="chunkHeight">The height of the chunks in pixels. Must be greater zero (0).</param>
		/// <param name="planarConfig">The planar configuration to use. Single-channel images must be continuously.</param>
		public ChunkedImage16(int width, int height, bool RGB, bool extraChannel, ushort[] initColor, int chunkHeight = 256, PlanarConfiguration planarConfig = PlanarConfiguration.Continuously) :
		...
		/// <param name="chunkHeight">The height of the chunks in pixels. Must be greater zero (0).</param>
		/// <param name="planarConfig">The planar configuration to use. Single-channel images must be continuously.</param>
		public ChunkedImage16(int width, int height, bool RGB, bool extraChannel, long initColor = 0, int chunkHeight = 256, PlanarConfiguration planarConfig = PlanarConfiguration.Continuously) :
			base(width, height, RGB, extraChannel, initColor, width, chunkHeight, planarConfig)
		{
			// We just need to correct the StorageLayout value, every thing else is set up correctly in the TiledImage16 constructor.
			StorageLayout = NumberOfChunks > 1 ? StorageLayout.Chunked : StorageLayout.SingleChunked;
		...
		public Image16 GetChunk(int chunk)
		{
		...
		public Image16 GetChunk(int chunk, int channel)
		{

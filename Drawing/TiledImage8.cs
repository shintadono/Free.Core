...
{
	/// <summary>
	/// Describes a tiled image of 1, 3 (RGB) or 4 (RGBX) unsigned 8-bit integers per pixel.
	/// </summary>
	public class TiledImage8 : TiledImage
	{
		#region Const
		/// <summary>
		/// Return value for <see cref="ChannelTypes"/>, for images with one channel.
		/// </summary>
		protected static readonly ChannelType[] Intensity8 = new ChannelType[] { ChannelType.Byte };

		/// <summary>
		/// Return value for <see cref="ChannelTypes"/>, for images with tree channel.
		/// </summary>
		protected static readonly ChannelType[] RGB8 = new ChannelType[] { ChannelType.Byte, ChannelType.Byte, ChannelType.Byte };

		/// <summary>
		/// Return value for <see cref="ChannelTypes"/>, for images with four channel.
		/// </summary>
		protected static readonly ChannelType[] RGBX8 = new ChannelType[] { ChannelType.Byte, ChannelType.Byte, ChannelType.Byte, ChannelType.Byte };
		#endregion
		...
		/// this is an array of <b>TileCountX*TileCountY</b> arrays of <see cref="TileWidth"/>*<see cref="TileHeight"/>*<see cref="NumberOfChannels"/> <b>byte</b>s each.
		/// </para>
		/// <para>
		/// If the image's <see cref="PlanarConfiguration"/> is <see cref="PlanarConfiguration.Separated">separated</see>
		/// this is an array of <b>TileCountX*TileCountY</b>*<see cref="NumberOfChannels"/> arrays of <see cref="TileWidth"/>*<see cref="TileHeight"/> <b>byte</b>s each.
		/// </para>
		...
		/// </remarks>
		public byte[][] Bits; // [tiles or chunks (*channels)][TileSize or ChunkSize (*channels)]
		#endregion
		...
		/// <returns>The value (0xXXRRGGBB) of the pixel.</returns>
		public int this[int x, int y]
		{
			get
			{
				...
				return (ext << 24) | (red << 16) | (grn << 8) | blu;
			}
			set
			{
				if (x < 0 || x >= Width) throw new IndexOutOfRangeException(nameof(x) + " index must be greater or equal to zero (0), and smaller than the " + nameof(Width) + ".");
				...
				{
					Bits[yTile * NumberOfTileX + xTile][yInTile * TileWidth + xInTile] = (byte)value;
					return;
				}

				byte red = (byte)value;
				byte grn = (byte)(value << 8);
				byte blu = (byte)(value << 16);
				byte ext = (byte)(value << 24);
				if (PlanarConfiguration == PlanarConfiguration.Continuously)
				{
					var tileBits = Bits[yTile * NumberOfTileX + xTile];
					int pos = (yInTile * TileWidth + xInTile) * NumberOfChannels;
		...
		public byte this[int channel, int x, int y]
		{
			get
			{
		...
		public TiledImage8(int width, int height, bool RGB, bool extraChannel, bool init, int tileWidth = 256, int tileHeight = 256, PlanarConfiguration planarConfig = PlanarConfiguration.Continuously)
		{
			...
			ChannelTypes = RGB ? (extraChannel ? RGBX8 : RGB8) : Intensity8;
			...
			if (init)
			{
				...
				Bits = new byte[tiles][];
				for (int i = 0; i < tiles; i++) Bits[i] = new byte[tileSize];
			}
		}
		...
		void InitSingleChannelImage(byte intensity)
		{
		...
		void InitSeperatedImageRGB(byte red, byte green, byte blue)
		{
			int tiles = Bits.Length;
			int tileSize = TileWidth * TileHeight;
		...
		void InitSeperatedImageRGBX(byte red, byte green, byte blue, byte extra)
		{
			int tiles = Bits.Length;
			int tileSize = TileWidth * TileHeight;
		...
		void InitContinuousImageRGB(byte red, byte green, byte blue)
		{
			int tiles = Bits.Length;
		...
		void InitContinuousImageRGBX(byte red, byte green, byte blue, byte extra)
		{
			int tiles = Bits.Length;
		...
		public void Clear(params byte[] color)
		{
		...
		/// Sets all pixel with the specified value.
		/// </summary>
		/// <param name="color">The color. (Layout: 0xXXRRGGBB)</param>
		public void Clear(int color = 0)
		{
			switch (NumberOfChannels)
			{
				case 1: InitSingleChannelImage((byte)color); break;
				case 3:
					if (PlanarConfiguration == PlanarConfiguration.Continuously) InitContinuousImageRGB((byte)(color >> 16), (byte)(color >> 8), (byte)color);
					else InitSeperatedImageRGB((byte)(color >> 16), (byte)(color >> 8), (byte)color);
					break;
				case 4:
					if (PlanarConfiguration == PlanarConfiguration.Continuously) InitContinuousImageRGBX((byte)(color >> 16), (byte)(color >> 8), (byte)color, (byte)(color >> 24));
					else InitSeperatedImageRGBX((byte)(color >> 16), (byte)(color >> 8), (byte)color, (byte)(color >> 24));
					break;
				default: throw new Exception("Invalid " + nameof(NumberOfChannels) + ".");
			}
		}
		...
		public void ClearChannel(int channel, byte value = 0)
		{
		...
		void DrawIntoTile(int tileIndex, TiledImage8 source, int dstStartX, int dstStartY, int srcStartX, int srcStartY, int amountX, int amountY, DrawBlendMode blendMode)
		{
					...
					if (NumberOfChannels == 1)
					{
							...
							for (int y = 0; y < tileAmountY; y++)
							{
								...
								if (blendMode == DrawBlendMode.Overwrite)
								{
									...
									else
									{
										for (int x = 0; x < tileAmountX; x++)
										{
											bits[dst++] = (byte)((srcBits[src++] + srcBits[src++] + srcBits[src++]) / 3);
								...
								else if (blendMode == DrawBlendMode.SourceAlpha)
								{
									for (int x = 0; x < tileAmountX; x++)
									{
										...
										double alpha = srcBits[src++] / 255.0;
										bits[dst++] = (byte)(bits[dst] * (1 - alpha) + gry * alpha + 0.5);
									}
								}
								else if (blendMode == DrawBlendMode.MinusSourceAlpha)
								{
									for (int x = 0; x < tileAmountX; x++, src++)
									{
										double gry = (srcBits[src++] + srcBits[src++] + srcBits[src++]) / 3.0;
										double alpha = srcBits[src++] / 255.0;
										bits[dst++] = (byte)(bits[dst] * alpha + gry * (1 - alpha) + 0.5);
									}
								}
						...
						else //if (source.PlanarConfiguration == PlanarConfiguration.Separated)
						{
							...
							for (int y = 0; y < tileAmountY; y++)
							{
								...
								{
									for (int x = 0; x < tileAmountX; x++, src++)
										bits[dst++] = (byte)((srcBitsRed[src] + srcBitsGrn[src] + srcBitsBlu[src]) / 3);
								}
								else if (blendMode == DrawBlendMode.SourceAlpha)
								{
									for (int x = 0; x < tileAmountX; x++, src++)
									{
										double alpha = srcBitsAlp[src] / 255.0;
										bits[dst++] = (byte)(bits[dst] * (1 - alpha) + (srcBitsRed[src] + srcBitsGrn[src] + srcBitsBlu[src]) * alpha / 3 + 0.5);
									}
								}
								else if (blendMode == DrawBlendMode.MinusSourceAlpha)
								{
									...
									{
										double alpha = srcBitsAlp[src] / 255.0;
										bits[dst++] = (byte)(bits[dst] * alpha + (srcBitsRed[src] + srcBitsGrn[src] + srcBitsBlu[src]) * (1 - alpha) / 3 + 0.5);
									}
								}
						...
						if (source.PlanarConfiguration == PlanarConfiguration.Continuously || source.NumberOfChannels == 1)
						{
							...
							for (int y = 0; y < tileAmountY; y++)
							{
								...
								else if (blendMode == DrawBlendMode.SourceAlpha)
								{
									for (int x = 0; x < tileAmountX; x++)
									{
										byte red = srcBits[src++]; byte grn = srcBits[src++]; byte blu = srcBits[src++]; byte alp = srcBits[src++];

										double alpha = alp / 255.0;
										bits[dst++] = (byte)(bits[dst] * (1 - alpha) + red * alpha + 0.5);
										bits[dst++] = (byte)(bits[dst] * (1 - alpha) + grn * alpha + 0.5);
										bits[dst++] = (byte)(bits[dst] * (1 - alpha) + blu * alpha + 0.5);
								...
								else if (blendMode == DrawBlendMode.MinusSourceAlpha)
								{
									for (int x = 0; x < tileAmountX; x++)
									{
										...
										double alpha = alp / 255.0;
										bits[dst++] = (byte)(bits[dst] * alpha + red * (1 - alpha) + 0.5);
										bits[dst++] = (byte)(bits[dst] * alpha + grn * (1 - alpha) + 0.5);
										bits[dst++] = (byte)(bits[dst] * alpha + blu * (1 - alpha) + 0.5);
										if (NumberOfChannels > 3) bits[dst++] = alp;
									}
								}
						...
						else //if (source.PlanarConfiguration == PlanarConfiguration.Separated)
						{
							...
							for (int y = 0; y < tileAmountY; y++)
							{
								...
								else if (blendMode == DrawBlendMode.SourceAlpha)
								{
									for (int x = 0; x < tileAmountX; x++, src++)
									{
										byte alp = srcBitsAlp[src];

										double alpha = alp / 255.0;
										bits[dst++] = (byte)(bits[dst] * (1 - alpha) + srcBitsRed[src] * alpha + 0.5);
										bits[dst++] = (byte)(bits[dst] * (1 - alpha) + srcBitsGrn[src] * alpha + 0.5);
										bits[dst++] = (byte)(bits[dst] * (1 - alpha) + srcBitsBlu[src] * alpha + 0.5);
										if (NumberOfChannels > 3) bits[dst++] = alp;
									}
								}
								else if (blendMode == DrawBlendMode.MinusSourceAlpha)
								{
									for (int x = 0; x < tileAmountX; x++, src++)
									{
										...
										double alpha = alp / 255.0;
										bits[dst++] = (byte)(bits[dst] * alpha + srcBitsRed[src] * (1 - alpha) + 0.5);
										bits[dst++] = (byte)(bits[dst] * alpha + srcBitsGrn[src] * (1 - alpha) + 0.5);
										bits[dst++] = (byte)(bits[dst] * alpha + srcBitsBlu[src] * (1 - alpha) + 0.5);
										if (NumberOfChannels > 3) bits[dst++] = alp;
									}
								}
							...
							for (int y = 0; y < tileAmountY; y++)
							{
								...
								else if (blendMode == DrawBlendMode.SourceAlpha)
								{
									for (int x = 0; x < tileAmountX; x++, dst++)
									{
										...
										double alpha = alp / 255.0;
										bitsRed[dst] = (byte)(bitsRed[dst] * (1 - alpha) + red * alpha + 0.5);
										bitsGrn[dst] = (byte)(bitsGrn[dst] * (1 - alpha) + grn * alpha + 0.5);
										bitsBlu[dst] = (byte)(bitsBlu[dst] * (1 - alpha) + blu * alpha + 0.5);
								...
								else if (blendMode == DrawBlendMode.MinusSourceAlpha)
								{
									for (int x = 0; x < tileAmountX; x++, dst++)
									{
										...
										double alpha = alp / 255.0;
										bitsRed[dst] = (byte)(bitsRed[dst] * alpha + red * (1 - alpha) + 0.5);
										bitsGrn[dst] = (byte)(bitsGrn[dst] * alpha + grn * (1 - alpha) + 0.5);
										bitsBlu[dst] = (byte)(bitsBlu[dst] * alpha + blu * (1 - alpha) + 0.5);
										if (bitsAlp != null) bitsAlp[dst] = alp;
									}
								}
							...
							for (int y = 0; y < tileAmountY; y++)
							{
								...
								else if (blendMode == DrawBlendMode.SourceAlpha)
								{
									for (int x = 0; x < tileAmountX; x++, src++, dst++)
									{
										...
										double alpha = alp / 255.0;
										bitsRed[dst] = (byte)(bitsRed[dst] * (1 - alpha) + srcBitsRed[src] * alpha + 0.5);
										bitsGrn[dst] = (byte)(bitsGrn[dst] * (1 - alpha) + srcBitsGrn[src] * alpha + 0.5);
										bitsBlu[dst] = (byte)(bitsBlu[dst] * (1 - alpha) + srcBitsBlu[src] * alpha + 0.5);
										if (bitsAlp != null) bitsAlp[dst] = alp;
									}
								}
								else if (blendMode == DrawBlendMode.MinusSourceAlpha)
								{
									for (int x = 0; x < tileAmountX; x++, src++, dst++)
									{
										...
										double alpha = alp / 255.0;
										bitsRed[dst] = (byte)(bitsRed[dst] * alpha + srcBitsRed[src] * (1 - alpha) + 0.5);
										bitsGrn[dst] = (byte)(bitsGrn[dst] * alpha + srcBitsGrn[src] * (1 - alpha) + 0.5);
										bitsBlu[dst] = (byte)(bitsBlu[dst] * alpha + srcBitsBlu[src] * (1 - alpha) + 0.5);
										if (bitsAlp != null) bitsAlp[dst] = alp;
									}
								}
		...
		/// </summary>
		/// <returns>The image (as <see cref="Image8"/>, <see cref="ChunkedImage8"/>, or <see cref="TiledImage8"/> depending on the
		/// original object) without the extra channel.</returns>
		public virtual TiledImage8 ExtraChannelRemoved()
		{
			...
			if (this is Image8) ret = new Image8(Width, Height, true, false, true, PlanarConfiguration);
			else if (this is ChunkedImage8) ret = new ChunkedImage8(Width, Height, true, false, true, ((ChunkedImage8)this).ChunkHeight, PlanarConfiguration);
			else ret = new TiledImage8(Width, Height, true, false, true, TileWidth, TileHeight, PlanarConfiguration);
		...
		public Image8 GetTile(int x, int y)
		{
			...
			var ret = new Image8(TileWidth, TileHeight, NumberOfChannels > 1, NumberOfChannels == 4, true, PlanarConfiguration);

			if (PlanarConfiguration == PlanarConfiguration.Continuously)
			{
				Buffer.BlockCopy(Bits[y * NumberOfTileX + x], 0, ret.Bits[0], 0, TileWidth * TileHeight * NumberOfChannels);
			}
			else
			{
				int tileSize = TileWidth * TileHeight;
				...
				{
					Buffer.BlockCopy(Bits[(y * NumberOfTileX + x) * NumberOfChannels + i], 0, ret.Bits[i], 0, tileSize);
				}
			}
		...
		public Image8 GetTile(int x, int y, int channel)
		{
			if (x < 0 || x >= NumberOfTileX) throw new ArgumentOutOfRangeException(nameof(x));
			if (y < 0 || y >= NumberOfTileY) throw new ArgumentOutOfRangeException(nameof(y));
			if (channel < 0 || channel >= NumberOfChannels) throw new ArgumentOutOfRangeException(nameof(channel));

			var ret = new Image8(TileWidth, TileHeight, false, false, true);
			...
			else Buffer.BlockCopy(Bits[(y * NumberOfTileX + x) * NumberOfChannels + channel], 0, ret.Bits[0], 0, tileSize);
